...
import { BaseLLMOutput, LLMOutputError, LLMError } from '../../llms/base.js';
import { Client, HttpError } from '@ibm-generative-ai/node-sdk';
...
class BAMLLMOutput extends BaseLLMOutput {
  static {
  ...
  constructor(content) {
    ...
    this.moderations = R.isArray(content.moderations) ? content.moderations : [
      content.moderations
    ].filter(R.isDefined);
  }
  static {
...
class BAMLLM extends LLM {
  ...
  async embed(input, options) {
    ...
    const results = await Promise.all(chunk(input, maxEmbeddingInputs).map(async (texts) => {
  ...
  loadSnapshot(snapshot) {
    ...
    Object.assign(this, snapshot, {
      client: snapshot?.client ?? new Client()
    });
  }
  // TODO: serialize?
  ...
  async *_stream(input, options, run) {
    try {
      ...
      yield* transformAsyncIterable(response[Symbol.asyncIterator](), this._rawResponseToOutput.bind(this));
    } catch (e) {
      throw this._transformError(e);
    }
  }
  ...
  _prepareParameters(overrides) {
    const guided = omitUndefined(overrides?.guided ? {} : this.parameters.guided ?? {});
    ...
    if (guidedOverride?.choice) {
      ...
      guided.json_schema = isString(guidedOverride.json) ? JSON.parse(guidedOverride.json) : guidedOverride.json;
    ...
    return {
      ...this.parameters,
      guided: isEmpty(guided) ? void 0 : guided
    };
  }
}

export { BAMLLM, BAMLLMOutput };
