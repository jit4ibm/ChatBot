...
function messagesToPromptFactory(rolesOverride = {}) {
  ...
  return (template) => {
    return utils_cjs.toBoundedFunction((messages) => {
...
function templateSchemaFactory(roles) {
  return zod.z.object({
    messages: zod.z.array(zod.z.object(remeda.mapToObj(roles, (role) => [
      role,
...
class LLMChatTemplates {
  ...
  static register(model, template, override = false) {
    if (model in this.registry && !override) {
      throw new errors_cjs.ValueError(`Template for model '${model}' already exists!`);
    }
  ...
  static get(model) {
    if (!this.has(model)) {
      throw new errors_cjs.ValueError(`Template for model '${model}' not found!`, [], {
        context: {
...
exports.LLMChatTemplates = LLMChatTemplates;
exports.messagesToPromptFactory = messagesToPromptFactory;
exports.templateSchemaFactory = templateSchemaFactory;
//# sourceMappingURL=llmChatTemplates.cjs.map
//# sourceMappingURL=llmChatTemplates.cjs.map