...
class CalculatorTool extends base_cjs.Tool {
  static {
...
Only use the calculator tool if you need to perform a calculation.`;
  ...
  inputSchema() {
    return zod.z.object({
      expression: zod.z.string().min(1).describe(`The mathematical expression to evaluate (e.g., "2 + 3 * 4"). Use Mathjs basic expression syntax. Constants only.`)
    });
  }
  limitedEvaluate;
  ...
  async _run({ expression }) {
    ...
    return new base_cjs.StringToolOutput(result);
  }
}
