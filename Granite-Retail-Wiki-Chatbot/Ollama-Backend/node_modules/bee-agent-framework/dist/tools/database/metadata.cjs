...
async function getMetadata(sequelize, provider, schema) {
  try {
    ...
    throw new base_cjs.ToolError(`Error initializing metadata: ${error}`, [], {
      isRetryable: false
    });
  }
}
  ...
  switch (provider) {
    ...
    case "oracle":
      return `
        ...
        `;
    default:
      throw new base_cjs.ToolError(`Provider ${provider} is not supported`, [], {
        isFatal: true,
        isRetryable: false
      });
  }
...
function getDefaultSchema(provider) {
  switch (provider) {
    ...
    case "db2":
    case "oracle":
      throw new base_cjs.ToolError(`Schema name is required for ${provider}`, [], {
        isRetryable: false,
        isFatal: true
      });
    default:
