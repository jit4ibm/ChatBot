...
function* recursiveSplitString(text, options) {
  ...
  const goodSplits = [];
  const [separator, ...remainingSeparators] = R.unique([
    ...options.separators ?? [],
    ""
  ]);
...
function compareVersion(a, comparator, b) {
  ...
  return R__namespace.isIncludedIn(diffNormalized, comparator);
}
...
function countSharedStartEndLetters(a, b) {
  if (!R.isString(a) || !R.isString(b)) {
    throw new errors_cjs.ValueError("Provided values must be all strings.");
  }
...
function findFirstPair(text, pair, options = {}) {
  ...
  if (!pair || !opening || !closing) {
    throw new errors_cjs.ValueError(`The "pair" parameter is required and must be non-empty!`);
  }
  let balance = 0;
  let startIndex = -1;
...
exports.countSharedStartEndLetters = countSharedStartEndLetters;
exports.findFirstPair = findFirstPair;
exports.halveString = halveString;
exports.isJsonLikeString = isJsonLikeString;
exports.mergeStrings = mergeStrings;
exports.recursiveSplitString = recursiveSplitString;
