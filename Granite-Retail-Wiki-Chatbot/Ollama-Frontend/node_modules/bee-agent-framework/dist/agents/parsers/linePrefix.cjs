...
class LinePrefixParser extends serializable_cjs.Serializable {
  static {
  ...
  constructor(nodes, options = {}) {
    super(), this.nodes = nodes, this.options = options, this.emitter = new emitter_cjs.Emitter({
      creator: this,
      namespace: [
        "agent",
        "parser",
    ...
    for (const [key, { next, isStart, isEnd }] of remeda.entries(nodes ?? {})) {
      hasStartNode = Boolean(hasStartNode || isStart);
      hasEndNode = Boolean(hasEndNode || isEnd);
      for (const nextKey of next) {
        if (key === nextKey) {
          throw new errors_cjs$1.ValueError(`Node '${key}' cannot point to itself.`);
        }
        if (!(nextKey in nodes)) {
          throw new errors_cjs$1.ValueError(`Node '${key}' contains a transition to non-existing node '${nextKey}'.`);
        }
      }
    }
    if (!hasStartNode) {
      throw new errors_cjs$1.ValueError(`At least one start node must be provided!`);
    }
    if (!hasEndNode) {
      throw new errors_cjs$1.ValueError(`At least one end node must be provided!`);
    }
  }
  fork(customizer) {
      ...
      if (parsedLine && !parsedLine.partial) {
        if (lastNode) {
          if (!lastNode.next.includes(parsedLine.key)) {
            ...
            this.throwWithContext(`Transition from '${this.lastNodeKey}' to '${parsedLine.key}' does not exist!`, errors_cjs.LinePrefixParserError.Reason.InvalidTransition, {
              line
            });
          }
          ...
          if (!this.options.waitForStartNode) {
            this.throwWithContext(`Parsed text line corresponds to a node "${parsedLine.key}" which is not a start node!`, errors_cjs.LinePrefixParserError.Reason.NotStartNode, {
              line
            });
          }
  ...
  throwWithContext(message, reason, extra = {}) {
    throw new errors_cjs.LinePrefixParserError([
  ...
  async end() {
    ...
    this.done = true;
    if (!this.lastNodeKey) {
      this.throwWithContext("Nothing valid has been parsed yet!", errors_cjs.LinePrefixParserError.Reason.NoDataReceived);
    }
    ...
    if (!currentNode.isEnd) {
      this.throwWithContext(`Node '${this.lastNodeKey}' is not an end node.`, errors_cjs.LinePrefixParserError.Reason.NotEndNode);
    }
  ...
  async emitPartialUpdate(data) {
    if (data.key in this.finalState) {
      this.throwWithContext(`Cannot update partial event for completed key '${data.key}'`, errors_cjs.LinePrefixParserError.Reason.AlreadyCompleted);
    }
  ...
  async emitFinalUpdate(key, field) {
    if (key in this.finalState) {
      this.throwWithContext(`Duplicated key '${key}'`, errors_cjs.LinePrefixParserError.Reason.AlreadyCompleted);
    }
    try {
      ...
      if (e instanceof zod.ZodError) {
        this.throwWithContext(`Value for '${key}' cannot be retrieved because it's value does not adhere to the appropriate schema.`, errors_cjs.LinePrefixParserError.Reason.InvalidSchema, {
          errors: [
            e
          ]
        });
  ...
  get normalizedNodes() {
    return remeda.entries(this.nodes).sort(([_, a], [__, b]) => a.prefix.length - b.prefix.length).map(([key, value]) => {
      return [
        key,
        {
  ...
  createSnapshot() {
    return {
      nodes: utils_cjs.shallowCopy(this.nodes),
      lines: utils_cjs.shallowCopy(this.lines),
...
exports.LinePrefixParser = LinePrefixParser;
...
//# sourceMappingURL=linePrefix.cjs.map
//# sourceMappingURL=linePrefix.cjs.map