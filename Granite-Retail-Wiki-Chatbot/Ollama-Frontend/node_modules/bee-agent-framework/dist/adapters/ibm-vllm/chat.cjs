...
class GrpcChatLLMOutput extends chat_cjs.ChatLLMOutput {
  static {
...
class IBMVllmChatLLM extends chat_cjs.ChatLLM {
  static {
    __name(this, "IBMVllmChatLLM");
  }
  emitter = new emitter_cjs.Emitter({
    namespace: [
      "ibm_vllm",
      "chat_llm"
    ],
  ...
  async *_stream(messages, options, run) {
    ...
    return yield* stream_cjs.transformAsyncIterable(response, (output) => new GrpcChatLLMOutput(output));
  }
  messagesToPrompt(messages) {
    return this.config.messagesToPrompt(messages);
  }
  static fromPreset(modelId, overrides) {
    const presetFactory = chatPreset_cjs.IBMVllmChatLLMPreset[modelId];
    if (!presetFactory) {
...
exports.GrpcChatLLMOutput = GrpcChatLLMOutput;
exports.IBMVllmChatLLM = IBMVllmChatLLM;
